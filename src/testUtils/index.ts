...
export const jestReset = o.extend(
  (): void => {
    ...
    facebook.setImplementation(
      new implementations.facebook.Facebook(undefined, "10.0")
    );
    ...
    reactiveStorage.setImplementation(
      implementations.reactiveStorage.reflectStorage
    );
    ...
    {
      const config: implementations.datetime.dateFnsWrapper.Configuration = {
        firstDayOfWeek: 0,
        locale: enUS,
        pm: true
      };
    ...
    {
      const config: implementations.handlePromise.promiseHandler.Configuration =
        {
          expectedDurations: {
            createDb: 1000,
            dbRequest: 1000,
    ...
    {
      const config: implementations.httpRequest.axiosWrapper.Configuration = {
        timeout: 30_000
      };

      implementations.httpRequest.axiosWrapper.configure(config);
      httpRequest.setImplementation(
        implementations.httpRequest.axiosWrapper.implementation
      );
    }

    {
      const config: implementations.testDelay.configurableTestDelay.Configuration =
        { enabled: false, timeout: 1000 };
  ...
  },
  {
    ...
    dictionary(
      ...
      definitions: Rec<LocaleName, implementations.lang.dictionary.Definitions>
    ): void {
      const config: implementations.lang.dictionary.Dictionary.Configuration = {
        localeName
      };
     ...
     */
    dom(this: void): void {
      const config: implementations.progressReporter.progressBar.Configuration =
        {
